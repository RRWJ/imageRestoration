...
from data.util import single2tensor3
class Model(nn.Module):
    def __init__(self, args, ckp):
        ...
        self.k = single2tensor3(np.expand_dims(np.float32(self.kernel), axis=2)).unsqueeze(0)
    ...
    def forward(self, x,idx_scale):
        ...
        if self.training:
            if self.n_GPUs > 1:
                ...
                return self.model(x, self.k, 4, self.sigma,n_iter = 6)
        else:
            if self.chop:
            ...
            if self.self_ensemble:
                ...
                return forward_function(x, self.k, 4, self.sigma,n_iter=6)
